 ...
 Future<bool> login(String email, String password) async {
  ...
  try {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      _errorMessage = errorData['message'] ?? errorData['error'] ?? 'Login failed. Please try again.';
      _isLoading = false;
      notifyListeners();
      return false; // Return false for failure
    }
  ...
  Future<void> _saveTokens(String accessToken, String refreshToken) async {
    try {
      ...
      print("Verification - Saved Access Token: ${savedAccess != null ? 'EXISTS' : 'MISSING'}");
      print("Verification - Saved Refresh Token: ${savedRefresh != null ? 'EXISTS' : 'MISSING'}");
...
Future<bool> forgotPassword(String email) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final Map<String, dynamic> errorData = json.decode(response.body);
      _errorMessage = errorData['message'] ?? errorData['error'] ?? 'Failed to send OTP';
      _isLoading = false;
      notifyListeners();
      return false;
    }
...
Future<bool> resendForgotOtp(String email) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      final Map<String, dynamic> errorData = json.decode(response.body);
      _errorMessage = errorData['message'] ?? errorData['error'] ?? 'Failed to resend OTP';
      _isLoading = false;
      notifyListeners();
      return false;
    }
...
Future<bool> resetPassword(String email, String newPassword) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      final Map<String, dynamic> responseData = json.decode(response.body);
      _errorMessage = responseData['message'] ?? 'Password reset successfully';
      _isLoading = false;
      notifyListeners();
      return true;
    } else {
      final Map<String, dynamic> errorData = json.decode(response.body);
      _errorMessage = errorData['message'] ?? errorData['error'] ?? 'Failed to reset password';
      _isLoading = false;
      notifyListeners();
      return false;
    }
