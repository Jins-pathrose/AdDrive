...
Future<void> loadRideState() async {
  try {
    ...
    if (savedActive && savedTripId != null) {
      ...
      print('Loaded ride state: active=$_isRideActive, trip_id=$_currentTripId');
  ...
  try {
    ...
    if (token != null && _currentTripId != null) {
      ...
      final response = await http.post(
        Uri.parse('http://192.168.1.48:3000/gps/pause'), // Update with your endpoint
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  try {
    ...
    if (postResponse.statusCode == 200 || postResponse.statusCode == 201) {
      ...
      if (data.containsKey('trip_id') && data['trip_id'] != null) {
        _currentTripId = data['trip_id'];
        // DON'T set _isRideActive here yet
        return {'success': true, 'trip_id': _currentTripId, 'existing': false};
      } else {
...
Future<bool> sendGpsUpdate(double latitude, double longitude) async {
  try {
    ...
    final endpoints = [
      'http://192.168.1.48:3000/gps/update',
    ];
...
Future<bool> startRideWithTracking(BuildContext context, String campaignId) async {
  try {
...
Future<void> fetchActiveCampaign() async {
  ...
  if (_weeklyUploadStatus != null) {
    final hasUploadedBefore = _weeklyUploadStatus!['has_uploaded_before'] ?? false;
...
Future<void> checkWeeklyUploadStatus(BuildContext context, String campaignId) async {
  try {
    final prefs = await SharedPreferences.getInstance();
    final token = prefs.getString('access_token');
